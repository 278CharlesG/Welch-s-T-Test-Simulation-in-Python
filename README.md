...
##  Project Overview
Using the "Red Wine Quality" dataset as a target, this program demonstrates how to use Python to explore the statistical relationship between chemical properties (such as volatile acidity) and sensory quality scores. It automates complex calculations, providing a streamlined workflow from raw data to statistical conclusions.
...
2.  **Descriptive Statistics**: Automatically extracts the **Mean**, **Median**, and **Quartiles (Q1, Q3, and IQR)** for specific columns.
...
| `matplotlib.pyplot` | For generating visualizations like Boxplots and QQ-Plots. |
| `scipy.stats` | For executing the T-test (`ttest_ind`) and generating probability plots (`probplot`). |
